...
def bulk_insert(offset, conn, C, snapshot, VCF, ANN, uniq, cnt):
    ...
    pipe.seek(0)
    C.copy_from(pipe, 'vcf_key', null = 'None') #FIXME: table hardcoded
    pipe.close()
    pipe = io.StringIO()
    ...
    pipe.seek(0)
    C.copy_from(pipe, 'vcf', null = 'None') #FIXME: table hardcoded
    pipe.close()
    #del pipe
    ...
    pipe.seek(0)
    C.copy_from(pipe, 'annotation', null = 'None') #FIXME: table hardcoded
    pipe.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument("-t", "--vcf_table_name", action = "store",
                     help = "the name of the target table in the database", default = 'vcf')
    ...
    C = conn.cursor()
    C.execute('SELECT MAX(key) FROM ebi.vcf_key')
    offset, = C.fetchall()[0]
    if offset is None:
        offset = 0
    else:
...
#                  on_bad_lines = 'error',
        ...
        if counter == args.batch_size:
            uniq = zip(ts, ena_run, integrity) if args.vcf_table_name == 'vcf' else None
            offset = bulk_insert(offset, conn, C, snapshot, VCF, ANN, uniq, counter)
            counter = 0
            VCF = []
            ANN = []
            ts = []
    ...
    if counter:
        uniq = zip(ts, ena_run, integrity) if args.vcf_table_name == 'vcf' else None
        bulk_insert(offset, conn, C, snapshot, VCF, ANN, uniq, counter)
