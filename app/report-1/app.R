...
new_cases <- tbl(con, "meta") %>%
  ...
  dplyr::rename(country_name="clean_country",
...
variant_master_table <-   new_cases %>%
  ...
  mutate (`Other variants` = weekly_sample-cases_with_variant_id,
...
ui <- dashboardPage(
    ...
    dashboardSidebar(
        sidebarMenu(id="sidebar",
                    menuItem("Samples from countries", icon = icon("map-marked-alt"), startExpanded = TRUE,
    ...
    dashboardBody(
        ...
        tabItems(
          ...
          tabItem(tabName = "country_map",
                  tabsetPanel( type = "tabs",
                               ...
                               tabPanel("EU", 
                                        ...
                                        #      )
                                        # ),
                                        #tags$p("This graphs updates itself automaticaly every day. The number of sequence derived from EBI, while the number of nex cases derived from ECDC")
          ...
          tabItem(tabName = "variants",
                  fluidRow(
                    column(width = 4,
                    selectInput("selected_country_for_variants", label = "Select country",
                                choices = unique(variant_master_table$country_name),
                                selected = "Netherlands"),
                    
                    checkboxInput("include_ecdc_new_case", label = "Include weekly new cases data from ECDC", value = FALSE),
                    ),
                    
                    column(width = 4,
                    radioButtons("vis_type", label = "Type of visualization",
                                 choices = c("normal","percent"),
                                 selected = "normal")
                    )
                  ),
            ...
            tabItem(tabName = "country_graph",
                    tabsetPanel( type = "tabs",
                       ...
                       tabPanel("Percent", 
                         tags$h4("The graph below shows how many percent of newly detected COVID-19 patient samples were sequenced and sent to EBI in a gived week in a given EU state"),        
                         box( title = "",
                             status = "primary",
                             height = "450", width = "12",solidHeader = FALSE,
                             column(width = 12,
                                   highchartOutput("pctPlot"),
                             )
                        ),
                        tags$p("This graphs updates itself automaticaly every day. The number of sequence derived from EBI, while the number of new cases derived from ECDC")
                       ...
                       tabPanel("Daily", 
                                ...
                                radioButtons("daily_log_lin", label = "",
                                             choices = c("linear", "logarithmic"),
                                             inline = TRUE, selected = "linear"),       
                                box( title = "",
                                     status = "primary",
                                     height = "450", width = "12",solidHeader = FALSE,
                                     column(width = 12,
            ...
            tabItem(tabName = "lineage_graph_country",
                    fluidRow(
                    ),
                    radioButtons("selected_country", label = "Select country",
            ...
            tabItem(tabName = "lineage_graph_lineage",
                    fluidRow(
                        radioButtons("selected_lineage", label = "Select VOC/VOI",
                        ...
                        box( title = "Lineage specific mutations on open and closed state S protein structure",
                             status = "primary",
                             height = "450", width = "12",solidHeader = FALSE,
                             fluidRow(
                                 column( width = 6,
                             ...
                             tags$p("Click on the 3D structures ", tags$a(href="https://www.rcsb.org/structure/7A95", "(PDB: 7A95)"), "and move your mouse to spin or use mouse wheel to zoom the protein for better view of the mutations")
...
server <- function(input, output) {
  ...
  output$eu_map <- renderHighchart({
    ...
    highchart() %>%
      hc_add_series_map(
        map, country_samples, value = "log_n_sample", joinBy = c('name','clean_country')    )  %>% 
  ...
  output$world_map <- renderHighchart({
    ...
    highchart() %>%
      hc_add_series_map(
        worldgeojson, country_samples, value = "log_n_sample", joinBy = c('name','clean_country')    )  %>% 
      # hc_colorAxis(stops = color_stops()) %>% 
      hc_legend(labelFormat = "", title=list(text="Number of samples in log10 scale") )%>%
    ...
    output$structure_open <- renderNGLVieweR({
        ...
        NGLVieweR("7A95") %>%
            ...
            addRepresentation("label", param=list(sele=':D AND 608',
                                                  labelType="format",
            ...
            addRepresentation("label", param=list(sele=x,
                                                  labelType="format",
            ...
            addRepresentation("label", param=list(sele=':A AND 1146',
                                                  labelType="format",
    ...
    output$structure_closed <- renderNGLVieweR({
        ...
        NGLVieweR("7A95") %>%
            ...
            addRepresentation("label", param=list(sele=':B AND 1146',
                                                  labelType="format",
    ...
    output$pctPlot <- renderHighchart({
        ...
        # con3 <- DBI::dbConnect(RPostgreSQL::PostgreSQL(), 
        #                        dbname=config$dbname,
        #                        host = config$host,
        #                        port = config$port,
        #                        user = config$user,
    ...
    output$distPlot <- renderHighchart({
        ...
        x <- tbl(con, "meta") %>%
            ...
            dplyr::mutate(clean_country= ifelse(clean_country=="USA", "United States", clean_country )) %>%
            dplyr::filter(!is.na(clean_collection_date))%>%
            group_by(clean_country, clean_collection_date) %>%
            dplyr::summarise(n=n()) %>%
            collect()%>%
            drop_na()%>%
            arrange(desc(n)) %>%
        ...
        x <- x%>%
            dplyr::filter(clean_collection_date>as_date("2020-01-06")) %>%
            dplyr::rename(Country="clean_country")%>%
            group_by(Country)%>%
     ...
     output$distPlot_lineage_country <- renderHighchart({
         ...
         highchart(type="stock")%>%
             ...
             hc_legend(enabled = TRUE,
                       title = list(text = "Select/deselect VOC/VOI:"))%>%
     ...
     output$variant_weekly <- renderHighchart({
       ...
       highchart() %>% 
         ...
         hc_title(text = paste("Cases in ", input$selected_country_for_variants, sep ="" )) %>% 
