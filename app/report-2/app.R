...
#   collect()
 ...
 app_human_meta_mv <- tbl(con, "app_human_meta") %>%
   collect()
          ...
          tabPanel(
            ...
            column( width = 4,
                    radioButtons("eu_graph_type",
                                 ...
                                 choices = c("Weekly sample number", "Cumulative sample number"),
          ...
          tabPanel(
            ...
            column( width = 4,
                    selectInput("world_select_country",
                                ...
                                selected = c("France", "Italy", "Egypt", "Spain", "Nigeria",
  ...
  output$eu_map <- renderHighchart({
    ...
    if (input$relative_to_population_eu) {
      highchart() %>%
        ...
        hc_title(text = "Number of raw MPox sequence from EU+UK") %>%
  ...
  output$world_map <- renderHighchart({
    ...
    if (input$relative_to_population_world) {
      highchart() %>%
        ...
        hc_legend(labelFormat = "", title = list(text = "Number of samples relative to 1 million citizen in log10 scale")) %>%
        hc_title(text = "Number of raw MPox sequence from worldwide") %>%
